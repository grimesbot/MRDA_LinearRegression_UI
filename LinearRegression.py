...
while (searchDate <= nextRankingDeadline):
    rankings_history[searchDate.strftime("%Y-%#m-%#d")] = get_rankings(searchDate)
    searchDate = searchDate + timedelta(weeks=1)

# Format ranking points and error to 2 decimal points
for dt in rankings_history.keys():
    for team in rankings_history[dt].keys():
        for key in rankings_history[dt][team]:
            rankings_history[dt][team][key] = round(rankings_history[dt][team][key], 2)

# Save rankings JSON to JavaScript file as linear_regression_ranking_history variable for local web UI
js_filename = "linear_regression_rankings.js"
# Delete if exists
...
json_filename = "linear_regression_rankings.json"
# Delete if exists
